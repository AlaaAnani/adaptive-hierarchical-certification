...
def certify(num_classes, sample, n0, n, sigma, tau, alpha, abstain=-1, parallel=False, correction='holm', kfwer_k=1):
    ...
    if parallel:
        with mp.Pool() as pool:
            c_ps = pool.map(map_test, [(sample[:, i], n0, n, num_classes, tau) for i in range(K) ]) # ((1100,), 100, 1000, 19, 0.75)
    else:
    ...
    return classes, ps, radius, (time_pvals, time_correction)
...
class Log:
    ...
    def __init__(self, name, log_name=None):
        self.log_dir = pathlib.Path(__file__).parent.absolute() / '..' / 'logs'
        now = datetime.now()

        rest = log_name if log_name else now.strftime("%Y_%m_%d_%H:%M:%S")
        self.folder_path = self.log_dir / (name + "_"  + rest)
