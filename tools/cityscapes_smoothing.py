...
class Certifier:
    ...
    def sample(self, image_np01, size, n, sigma, logits=False, do_tqdm=False):
    ...
    def process_precertify(self, batch, logits=False):
        ...
        baseline_pred, baseline_logits = self.sample(image_np01, size, n=1, sigma=0, logits=logits) # (1, 512, 1024)
    ...
    def fill_stats_h(self, stats, name, n, n0, i, classes_certify, label_i, non_ignore_idx, edge_map):
        ...
        ig_per_class_dict = self.ig_per_class(c_ig_map, np.zeros(512*1024), classes_certify, label_i.reshape(1, -1), non_ignore_idx)
    ...
    def test(self, calibration=False, temperature=1.14):
        ...
        for batch in tqdm(self.testloader, desc='Main Loop'):
            ...
            image_np01 = image[0].numpy().transpose((1, 2, 0)).astype(np.float32).copy()/255.0 # (1024, 2048, 3)
            size = label.size()
            baseline_pred, baseline_logits = self.sample(image_np01, size, n=1, sigma=0, logits=True) # (1, 512, 1024)
            flat_label = label.flatten().cpu().numpy()
            non_ignore_idx = flat_label != 255
    ...
    def test_calibration(self):
        ...
        self.log('end of test_calibration')
    ...
    def main_skeleton(self):
        ...
        with torch.no_grad():
            for batch in tqdm(self.testloader, desc='Main Loop'):
                ...
                name, image_np01, baseline_pred, size, labels_h, non_ignore_idx, edge_map, label, baseline_logits = self.process_precertify(batch, logits=False)
                ...
                for n in list(reversed(sorted(self.n))):
                    ...
                    stats_name, stats = self.exp_adaptive_n0_and_thresholds(name, stats,
                                      samples_flattened,
                                      samples_flattened_h, labels_h, 
                                      non_ignore_idx, edge_map,
                                      samples_logits,
                                      label, baseline_pred)
                pickle.dump(stats, open(f'{self.default_log_dir}/paper_logs/{stats_name}/{self.steps}.pkl', 'wb'))
                print(f'Saving {self.default_log_dir}/paper_logs/{stats_name}/{self.steps}.pkl')
                self.steps +=1
    ...
    def exp_adaptive_n0_and_thresholds(self, name, stats,
                                      ...
                                      samples_logits,
                                      label, baseline_pred):
        stats_name = 'adaptive_n0_and_thresholds'
        os.makedirs(f'{self.default_log_dir}/paper_logs/{stats_name}/', exist_ok=True)
