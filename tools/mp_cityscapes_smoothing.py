...
class Certifier:
    ...
    def get_samples(self, image_np01, n, size, name, load=False):
        if load:
            ...
            _, logits = self.sample(image_np01, size, self.max_n, sigma = self.sigma, logits=True)
    ...
    def main_skeleton(self):
        ...
        with torch.no_grad():
            for batch in tqdm(self.testloader, desc='Main Loop'):
                ...
                stats[name] = {}
                for n in tqdm(list(reversed(sorted(self.n)))):
