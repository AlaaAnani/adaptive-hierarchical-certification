...
class Certifier:
    ...
    def parse_args(self):
        ...
        parser.add_argument("--config", default='/BS/mlcysec2/work/robust-segmentation/code/hrnet_seg/configs/acdc.ini', is_config_file=True)
    ...
    def add_argparse_args(cls, parser):
        ...
        parser.add_argument('--default_log_dir', type=str, default= '/BS/mlcysec2/work/robust-segmentation/code/hrnet_seg/acdc1024')
        ...
        parser.add_argument('--cfg', type=str, default='/BS/mlcysec2/work/robust-segmentation/code/hrnet_seg/configs/acdc.yml')
    ...
    def fast_certify(self, samples_flattened, n0, n):
        ...
        return modes
    def adapt_to_hierarchies(self, samples_flattened, diff):
        diff = diff.flatten()
        hierarchies = np.zeros_like(diff)
        hierarchies[diff < 0.85] = 1
        hierarchies[diff < 0.5] = 2
    ...
    def info_gain(self, h_i, classes_certify, label_i):
        num_pixels = len(classes_certify)
        c_idx = classes_certify != 19
        if h_i == 0: 
            info_gain = (classes_certify[c_idx] == label_i[c_idx]).sum()*np.log(19)
        else:
            classes_certify_certified = classes_certify[c_idx]
            tmp = self.lookup_info_gain[h_i - 1][classes_certify_certified]
            info_gain = (classes_certify[c_idx][tmp == 0] == label_i[c_idx][tmp == 0]).sum()*np.log(19)
            info_gain += (np.log(19)-np.log(tmp[tmp!=0])).sum()
        return info_gain
    ...
    def info_gain_adaptive(self, classes_certify, hierarchy_map, labels_h):
        ...
        for i, label in enumerate(labels_h):
            ...
            info_gain += self.info_gain(i, classes_certify[h_idx], label.flatten()[h_idx])
        return info_gain
        ...
        with torch.no_grad():
            for batch in tqdm(self.testloader, desc='Main Loop'):
                ...
                for n in tqdm(self.n, desc='ns'):
                    ...
                    samples_flattened = samples.reshape(n, -1) # (N, w*h)
                    ...
                    samples_flattened_adaptive, hierarchy_map = self.adapt_to_hierarchies(samples_flattened, diff)
                    classes_certify = self.fast_certify(torch.tensor(samples_flattened_adaptive), n0, n)
